...
if(!empty($data) && isset($data[$thread]) && isset($data[$message_id])) {
    ...
    $reply_url_template_quote = phorum_get_url(PHORUM_REPLY_URL, '%thread_id%', '%message_id%','quote=1');
			...
			//   RNCA
			//
            $rnca_url_template          = "rnca.php?" ."%action_id%, "  . "%message_id%, " . "%thread_id%";
			//
			//
			//
	...
	// RNCA
        //
        $URLS["recommend_message"] = str_replace(array('%action_id%','%message_id%'),array(PHORUM_REOPEN_THREAD,$thread),$recommend_url_template);
		//
		//
		//   
    }
	...
	//
        ...
        if($PHORUM["DATA"]["MODERATOR"]) {
		   ...
		   //
			...
			$row["URL"]["COPY_MESSAGE"] = "rnca.php?action_id="   .PHORUM_COPY_MESSAGE ."&thread=".$row["thread"] ."&message_id=" & $row["message_id"];
			$row["URL"]["STICK_MESSAGE"] = "rnca.php?action_id="   .PHORUM_STICK_MESSAGE ."&thread=".$row["thread"] ."&message_id=" & $row["message_id"];
			...
			//RNCA
			//
			$row["URL"]["RECOMMEND_MESSAGE"] = $URLS["recommend_message"];
			//
			//
			//
        }
       ...
       //         } else
                  if(is_numeric(str_replace(".", "", $row["ip"]))){
...
//
        ...
        $PHORUM["DATA"]["LANG"]["eric_read_subject"]=$PHORUM["DATA"]["MESSAGE"]["subject"] ;
	$PHORUM["DATA"]["LANG"]["eric_read_author"]= 	$PHORUM["DATA"]["LANG"]["eric_author"];
        $PHORUM["DATA"]["LANG"]["eric_read_authorname"]=$PHORUM["DATA"]["MESSAGE"]["author"];
//
//
//
	$PHORUM["DATA"]["LANG"]["eric_read_body"]=$PHORUM["DATA"]["MESSAGE"]["body"];
	if ($PHORUM["DATA"]["MESSAGE"]["attachments"]){
	      foreach($PHORUM["DATA"]["MESSAGE"]["attachments"] as $rnca_row){
                $PHORUM["DATA"]["LANG"]["eric_read_body"] = $PHORUM["DATA"]["LANG"]["eric_read_body"]
	          . "<br /><br /><br /><a href=\"" 
                  . $rnca_row["url"]
                  . "\"><font color=\"blue\" size=\"4\">"
		  . $PHORUM["DATA"]["LANG"]["AttachOpen"]
                  . "("
                  . $rnca_row["size"]
                  . ")"
		  . "</font></a>&nbsp;&nbsp;";
...
// print_var($PHORUM["DATA"][MESSAGES]);
