...
<table border="1" style="width:950px;background-color:#A9D0F5;border-spacing:0px;padding:0px; text-align:center;">
  ...
  <div style="width:100%; height:80%; background-color:#A9D0F5;border-width:0px; border-spacing:0px;padding:0px; text-align: left; ">
    ...
    <tr>
    <td td class="PhorumTableRowAlt" style="font-family:Arial, Helvetica, sans-serif;font-size: 14px; font-weight:300; color: blue">{LANG->Sticky}
	     <a href="{MESSAGES_STICKY->URL->READ}"  title="{title}">
               <font color="darkblue">{MESSAGES_STICKY->subject}</font></a>
		...
		{/LOOP MESSAGES_STICKY->attachments}
            ...
            {ELSEIF FORUM_ID 17}
            {ELSE}   
                {MESSAGES_STICKY->author}
             {/IF}
             {IF MESSAGES_STICKY->URL->PROFILE}</a>{/IF}
             
             <span style="color:blue; font-size:12px; font-family:Arial, Helvetica, sans-serif">
             ...
             {IF MODERATOR true}
                 ...
                 {ELSE}
    		      ...
    		      &nbsp; <a title="{LANG->eric_RecommendMessage}" href="{MESSAGES_STICKY->URL->RECOMMEND_MESSAGE}"><font color="gray" size="2">{LANG->eric_RecommendMessage}</font></a>
                   {/IF}
                 {/IF}
             {/IF} 
       </td></tr>
    ...
    <tr>
    <td class="PhorumTableRow" style="padding-left: {MESSAGES->indent_cnt}px;padding-bottom:10pt;">
         {IF FORUM_ID 29}
         ...
         {ELSE}
                <img src="{URL->TEMPLATE}/images/{icon}.png" class="icon1616" alt="{alt}" title="{title}" />
 	            <a href="{MESSAGES->URL->READ}"  title="{title}">
                    {IF MESSAGES->parent_id 0} 
                          {MESSAGES->subject}  ({MESSAGES->size} {LANG->rnca_char})
                    {ELSE}
                          {MESSAGES->subject} ({MESSAGES->size} {LANG->rnca_char})
                    {/IF}
                    </a> 
		...
		{/LOOP MESSAGES->attachments}
          ...
          {IF MODERATOR true}
              ...
              {IF MESSAGES->moved}
                <a title="{LANG->DeleteThread}" href="{MESSAGES->URL->DELETE_MESSAGE}">
                   <font color="gray" size="2">{LANG->DeleteThread}</font>
                </a>
              {ELSE}
                {IF MESSAGES->threadstart true}
                    &nbsp; <a title="{LANG->MoveThread}" href="{MESSAGES->URL->MOVE}"><font color="gray" size="2">{LANG->MoveThread}</font></a>
                    &nbsp; <a title="{LANG->MergeThread}" href="{MESSAGES->URL->MERGE}"><font color="gray" size="2">{LANG->MergeThread}</font></a>
                    &nbsp; <a title="{LANG->DeleteThread}" href="{MESSAGES->URL->DELETE_THREAD}"><font color="gray" size="2">{LANG->DeleteThread}</font></a>
         		    &nbsp; <a title="{LANG->eric_RecommendMessage}" href="{MESSAGES->URL->RECOMMEND_MESSAGE}"><font color="gray" size="2">{LANG->eric_RecommendMessage}</font></a>
                {ELSE}
                    &raquo; <a title="{LANG->DeleteMessage}" href="{MESSAGES->URL->DELETE_MESSAGE}"><font color="gray" size="2">{LANG->DeleteMessage}</font></a>
                {/IF}
