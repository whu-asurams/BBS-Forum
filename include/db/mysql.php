...
function phorum_db_get_thread_list($page, $include_bodies=FALSE)
{
    ...
    $messagefields =
       'author, datestamp, email, message_id, forum_id, meta,
        ...
        subject, CHARACTER_LENGTH(body), thread, thread_count, user_id, viewcount, threadviewcount,
        closed, recent_message_id, recent_user_id, recent_author';
...
function phorum_db_get_thread_list_rnca1($page, $include_bodies=FALSE)
{
    $PHORUM = $GLOBALS['PHORUM'];
    ...
    // The messagefields that we want to fetch from the database.
    $messagefields =
       'author, datestamp, email, message_id, forum_id, meta, modifystamp,
...
function phorum_db_get_thread_list_rnca2($page, $include_bodies=FALSE)
{
    $PHORUM = $GLOBALS['PHORUM'];
            ...
            case 'stickies':
                ...
                $sql = "(SELECT $messagefields
                        ...
                        WHERE  status=".PHORUM_STATUS_APPROVED." AND
                               parent_id=0 AND
                               sort=1 AND
                               forum_id={$PHORUM['forum_id']}) UNION 
                        (SELECT $messagefields
                        FROM   {$PHORUM['message_table']}
                        WHERE  status=".PHORUM_STATUS_APPROVED." AND
                               parent_id=0 AND
