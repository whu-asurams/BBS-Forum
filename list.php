...
$list_page_url_template = phorum_get_url_rnca(PHORUM_LIST_URL, '%forum_id%', 'page=%page_num%');
...
if($rows == null) {
    ...
    if ($PHORUM["threaded_list"]){
        ...
        $read_url_template = phorum_get_url_rnca(PHORUM_READ_URL, '%thread_id%','%message_id%');
        ...
        foreach($rows as $key => $row){
            ...
            if($PHORUM["count_views"]) {  // show viewcount if enabled
                if($PHORUM["count_views"] == 2) { // viewcount as column
                  ...
                  //  $rows[$key]["author"]=$row["author"]." ({$row['viewcount']} " . $PHORUM['DATA']['LANG']['Views_Subject'] . ")";
                }
            }
            $row["ip"] = "127.1.0.0";
	...
	//
            ...
            if($rows[$key]["sort"] >100)
                 unset($rows[$key]);
            if($rows[$key]["sort"] == 1){
        //       ||$rows[$key]["sort"] == 6
        //       ||$rows[$key]["sort"] == 7||$rows[$key]["sort"] == 8
        //       ||$rows[$key]["sort"] == 9){
               $rowsSticky[$key] = $rows[$key];
               unset($rows[$key]);
            }elseif($rows[$key]["sort"] > 2){
              // unset($rows[$key]);
            }else{}
            $rows[$key]["size"] = $rows[$key]["CHARACTER_LENGTH(body)"] - 2;
	//
	//
	//
        ...
        $read_url_template        = phorum_get_url_rnca(PHORUM_READ_URL, '%thread_id%');
        $newpost_url_template     = phorum_get_url_rnca(PHORUM_READ_URL, '%thread_id%','gotonewpost');
        $read_page_url_template   = phorum_get_url_rnca(PHORUM_READ_URL, '%thread_id%','page=%page_num%');
        $recent_page_url_template = phorum_get_url_rnca(PHORUM_READ_URL, '%thread_id%','%message_id%','page=%page_num%');
        $recent_url_template      = phorum_get_url_rnca(PHORUM_READ_URL, '%thread_id%','%message_id%');
...
if($PHORUM['DATA']['LOGGEDIN']) {
    ...
    if($PHORUM["DATA"]["MODERATOR"]){
		...
		//  RNCA
		//
		$recommend_url_template  = phorum_get_url(PHORUM_MODERATION_URL, PHORUM_RECOMMEND_MESSAGE, '%message_id%');
		//
		//
		//
    ...
    foreach($rowsSticky as $key => $row){ 
       if($PHORUM["DATA"]["MODERATOR"]){
           $rowsSticky[$key]["URL"]["DELETE_THREAD"]  = str_replace('%message_id%',$row['message_id'],$delete_thread_url_template);
           $rowsSticky[$key]["URL"]["MERGE"]          = str_replace('%message_id%',$row['message_id'],$merge_thread_url_template);;
 			//
			// RNCA
			//
            $rowsSticky[$key]["URL"]["RECOMMEND_MESSAGE"] = str_replace('%message_id%',$row['message_id'],$recommend_url_template);
			//
			//
			//
        }
    ...
    foreach($rows as $key => $row){
        ...
        if($PHORUM["DATA"]["MODERATOR"]){
			...
			// RNCA
			//
            $rows[$key]["URL"]["RECOMMEND_MESSAGE"] = str_replace('%message_id%',$row['message_id'],$recommend_url_template);
			//
			//
			//
...
$PHORUM["DATA"]["MESSAGES_STICKY"] = $rowsSticky;
