...
//
    ...
    case PHORUM_RECOMMEND_MESSAGE: // this is the first step of recommend a message
        ...
        $PHORUM['DATA']["FORM"]["forum_id"]=$PHORUM["forum_id"];
        $PHORUM['DATA']["FORM"]["thread_id"]=$msgthd_id;
        $PHORUM['DATA']["FORM"]["mod_step"]=PHORUM_DO_MESSAGE_RECOMMEND;

     
        $PHORUM['DATA']["FORM"]["subject"] =htmlspecialchars($message["subject"], ENT_COMPAT, $PHORUM["DATA"]["HCHARSET"]);
		$PHORUM['DATA']["FORM"]["author"] =htmlspecialchars($message["author"], ENT_COMPAT, $PHORUM["DATA"]["HCHARSET"]);
		$PHORUM['DATA']["FORM"]["body"] =$message["body"];
        ...
        $template="recommend_message";

        break;

    case PHORUM_DO_MESSAGE_RECOMMEND: // this is the last step of a message move
        $updateMessage = array();
        ...
        $recommend=$_POST['group1'];
		$recommendStr = 2;		
        switch($recommend){
		   case "sticky":
		       $recommendStr = PHORUM_SORT_STICKY;
		       break;
		   case "topten":
 		       $recommendStr = 3;
			   break;
		...
		if($message["sort"] != $recommendStr){
              $updateCheck ++;
	    }

	 if($updateCheck > 0){
            //  if($_SESSION["version"] == 1)        
	//	   $updateMessage["sort"] = $recommendStr;
        //      elseif($_SESSION["version"] == 2){        
        //	   $updateMessage["sort"] = $recommendStr + 100;
        //      }else{
		   $updateMessage["sort"] = $recommendStr;
        //      }
	      phorum_db_update_message($msgthd_id, $updateMessage);
        }
